...
local function drawBottomPanel(canSpin, hasBets, state)
    ...
    if phase == "results" then
        ...
        elseif state.payout and state.payout - totalBetAmount > 0 then
            writeAt(rx + 10, panelY + 1, "WON +" .. state.payout - totalBetAmount .. " \x13", C.win, C.header)
        else
            writeAt(rx + 10, panelY + 1, "LOST " .. state.payout - totalBetAmount .. " \x15", C.loss, C.header)
        end
    end
