...
local function drawWidescreenBoard(currentBets)
    ...
    local function getBetLabel(val)
        ...
        elseif val < 10 then return "[" .. val .. " ]" else return "[" .. val .. "]" end
    end
    ...
    local dozW = 19
    local dozens = {
        {lbl = "1st 12", act = "doz1"}, {lbl = "2nd 12", act = "doz2"}, {lbl = "3rd 12", act = "doz3"}
    }
...
local function drawRealWheel(activeNumber, phase, tick, payout, hasBets)
    ...
    local wx, wy = W - wheelW - 4, 5
    tick, payout = tick or 0, payout or 0
    ...
    for i = 2, wheelH - 3 do writeAt(wx + 10, wy + i, pattern[1], C.text, C.wheelInner) end
    writeAt(wx + 3, wy + 9, string.rep(pattern[3], 5) .. "[   ]" .. string.rep(pattern[3], 5), C.text, C.wheelInner)

    if activeNumber then
        local baseIdx = 1
        for idx, val in ipairs(WHEEL_STRIP) do if val == activeNumber then baseIdx = idx break end end
        ...
        writeAt(wx + 7, wy + 9, "> " .. midStr .. " <", (phase == "results" and C.text or C.chipGold), getNumberColor(centerNum))
        writeAt(wx + 9, wy + 11, botStr, C.text, getNumberColor(nextNum))
    end

    local statusStr = (phase == "spinning" and "SPINNING...") or (phase == "results" and "CLICK ANYWHERE TO RESTART") or "PLACE BETS"
    centreInZone(wx, wx + wheelW - 1, wy + wheelH + 1, statusStr, (phase == "spinning" and C.chipGold or C.text), C.felt)
end

local function drawBottomPanel(canSpin, hasBets, state)
    local panelY, btnW, btnH = H - 4, 16, 3
    ...
    writeAt(tx, panelY + 1, "- You can click multiple time to cycle between wages", C.text, C.felt)
    ...
    if phase == "results" then
        ...
        writeAt(rx + 5, panelY + 1, formattedNum, (c == C.numGreen and colours.black or C.text), c)

        local totalBet = 0
        for _, val in pairs(state.bets) do totalBet = totalBet + (val or 0) end

        if not hasBets then writeAt(rx + 10, panelY + 1, "  NO BETS", colours.lightGrey, C.header)
        else
            local net = (state.lastPayout or 0) - totalBet
            if net > 0 then writeAt(rx + 10, panelY + 1, "WON +" .. net .. " \x13", C.win, C.header)
            elseif net < 0 then writeAt(rx + 10, panelY + 1, "LOST -" .. math.abs(net) .. " \x15", C.loss, C.header)
            else writeAt(rx + 10, panelY + 1, "PUSH +0", colours.lightGrey, C.header) end
        end
    end

    local sx = W - btnW - 4
    if canSpin and hasBets and phase == "betting" then
        makeButton(sx, panelY, btnW, btnH, "spin", " SPIN WHEEL \x10 ", "", C.btnSpin, C.btnText, C.btnText)
    else
...
function ROULETTE_UI.getSize() return W, H end
...
function ROULETTE_UI.draw(state)
    ...
    drawRealWheel((state.phase == "spinning" and state.activeSpinNumber or state.winningNumber), 
                  (state.phase or "betting"), state.spinTick or 0, state.lastPayout or 0, true)
    drawBottomPanel((state.queueChips or 0) >= 0, true, state)
end
